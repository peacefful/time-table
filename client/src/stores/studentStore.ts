...
import { directors, groups, institutions, students, users } from '@/API/api-enterprises-institutions'
import axios from "axios"

export const useStudentStore = defineStore('studentStore', () => {
	async function getDirector(arrayData:object[], ownValueFind:string|number) {
		try {
			const data:object[] = (await axios.get(directors)).data
			arrayData.value = data.find(director => director.institution.appellation === ownValueFind.value)
		} catch (error) {
			console.log(error);
		}
	}

	async function getStudentData(user:object[], group:string) {
		try {
	...
	async function getStudentSchedules(schedulesData:object[]) {
		try {
	...
	async function getStudents(studentsData:object[]) {
		try {
	...
	async function getGroup(groupsDatas:object[]) {
		try {
			const data:object[] = (await axios.get(groups)).data
	...
	return { getDirector, getStudentData, getStudentSchedules, getStudents, getGroup }
})